...
static void add_source(unsigned int n, float *x, const float *s, float dt) {
  ...
  const __m128 pdt = fset1(dt);
  unsigned int i;
  for (i = 0; i < size - 4; i += 4) {
    __m128 px = faload(&x[i]);
    __m128 ps = faload(&s[i]);
    __m128 product = ffmadd(pdt, ps, px);  // x + dt * s[i]
    fastore(&x[i], product);                // x[i] += dt * s[i];
  }
...
static void lin_solve_rb_step(grid_color color, unsigned int n, float a,
  ...
  const __m128 pinvc = fset1(invc);
  const __m128 pa = fset1(a);
  ...
  for (unsigned int y = 1; y <= n; ++y, start = 1 - start) {
    ...
    for (unsigned int x = start; x < width - (1 - start); x += 4) {
      ...
      __m128 f = fload2x4(&same0[index]);
      __m128 u = fload2x4(&neigh[index - width]);
      __m128 r = fload2x4(&neigh[index - start + 1]);
      __m128 d = fload2x4(&neigh[index + width]);
      __m128 l = _mm_blend_ps(fshl(r), fset1(leftmost), 0b0001);

      // t = (f + a * (u + r + d + l)) / c
      __m128 t = fmul(ffmadd(pa, fadd(u, fadd(r, fadd(d, l))), f), pinvc);
      fstore(&same[index], t);

      // extract the rightmost to be the next leftmost
      _MM_EXTRACT_FLOAT(leftmost, r, 3);
    }
  }
}
...
static void advect_rb(grid_color color, unsigned int n, float *samed,
  ...
  const __m128i psuccs = iset(3, 2, 1, 0);
  const __m128 pdt0 = fset1(dt0);
  const __m128 plowest = fset1(0.5f);
  const __m128 phighest = fset1(n + 0.5f);
  const __m128i pone = iset1(1);
  const __m128 pfone = fset1(1.0);
  const __m128i pnplus2 = iset1(n + 2);
  const __m128i pwidth = iset1(width);
  const __m128i phalfgrid = imul(pnplus2, pwidth);
  for (int iy = 1; iy <= (int)n; iy++, shift = -shift, start = 1 - start) {
    const __m128i pshift = iset1(shift);
    const __m128i pstart = iset1(start);
    const __m128i pi = iset1(iy);
    for (int ix = start; ix < width - (1 - start); ix += 4) {
      __m128i pj = iadd(iset1(ix), psuccs);  // j = x + 0, ..., x + 7

      int index = idx(ix, iy, width);
      const __m128i pgridi = pi;
      const __m128 pfgridi = itof(pgridi);  // (float)gridi
      const __m128i pgridj =                // 2 * j + shift + start
          iadd(_mm_slli_epi32(pj, 1), iadd(pshift, pstart));
      const __m128 pfgridj = itof(pgridj);  // (float)gridj
      const __m128 psameu0 = fload(&sameu0[index]);
      const __m128 psamev0 = fload(&samev0[index]);
      __m128 px = ffnmadd(pdt0, psameu0, pfgridj);  // gridj - dt0 * sameu0[i]
      __m128 py = ffnmadd(pdt0, psamev0, pfgridi);  // gridi - dt0 * samev0[i]
      ...
      const __m128i pj0 = ftoi(px);  // j0 = (int)x;
      const __m128i pi0 = ftoi(py);  // i0 = (int)y;

      const __m128 ps1 = fsub(px, itof(pj0));  // s1 = x - j0;
      const __m128 ps0 = fsub(pfone, ps1);     // s0 = 1 - s1;
      const __m128 pt1 = fsub(py, itof(pi0));  // t1 = y - i0;
      const __m128 pt0 = fsub(pfone, pt1);     // t0 = 1 - t1;

      // Let's build IX(j0, i0):
      const __m128i pisoddrow = iand(pi0, pone);         // i % 2
      const __m128i pisevenrow = ixor(pisoddrow, pone);  // !isoddrow

      // (!= parity) === (j0 % 2) ^ (i0 % 2) === (j0 & 1) ^ (i0 & 1)
      const __m128i pisblack = ixor(iand(pj0, pone), pisoddrow);
      const __m128i pisred = ixor(pisblack, pone);     // !isblack
      const __m128i pfshift = isub(pisred, pisblack);  // isred ? 1 : -1
      ...
      const __m128i p_starts_at_zero =
      ...
      const __m128i pbase = imul(pisblack, phalfgrid);
      const __m128i poffset = iadd(   // (j0 / 2) + i0 * ((n+2) / 2)
          _mm_srai_epi32(pj0, 1),  // (j0 / 2)
          imul(pi0, pwidth)           // i0 * ((n+2) / 2)
      );

      // i0j0 = // IX(j0, i0)
      const __m128i pi0j0 = iadd(pbase, poffset);
      // i1j1 = i0j0 + width + (1 - isoffstart);
      const __m128i pi1j1 = iadd(pi0j0, iadd(pwidth, p_starts_at_zero));
      ...
      const __m128i pi1j0 = iadd(pi0j0, iadd(pwidth, imul(pfshift, phalfgrid)));
      ...
      const __m128 pd0i0j0 = fgather(d0, pi0j0, 4);
      const __m128 pd0i0j1 = fgather(d0, pi0j1, 4);
      const __m128 pd0i1j0 = fgather(d0, pi1j0, 4);
      const __m128 pd0i1j1 = fgather(d0, pi1j1, 4);
      ...
      const __m128 pu0i0j0 = fgather(u0, pi0j0, 4);
      const __m128 pu0i0j1 = fgather(u0, pi0j1, 4);
      const __m128 pu0i1j0 = fgather(u0, pi1j0, 4);
      const __m128 pu0i1j1 = fgather(u0, pi1j1, 4);
      ...
      const __m128 pv0i0j0 = fgather(v0, pi0j0, 4);
      const __m128 pv0i0j1 = fgather(v0, pi0j1, 4);
      const __m128 pv0i1j0 = fgather(v0, pi1j0, 4);
      const __m128 pv0i1j1 = fgather(v0, pi1j1, 4);
...
static void project_rb_step1(unsigned int n, grid_color color,
  ...
  const __m128 zeros = fset1(0.0f);
  for (unsigned int i = 1; i <= n; ++i, start = 1 - start)
    for (unsigned int j = start; j < width - (1 - start); j += 4)
      fstore(&sameu0[idx(j, i, width)], zeros);

  const __m128 ratio = fset1(-0.5f / n);
  for (unsigned int i = 1; i <= n; ++i, start = 1 - start) {
    for (unsigned int j = start; j < width - (1 - start); j += 4) {
      int index = idx(j, i, width);
      __m128 u = fload2x4(&neighv[index - width]);
      __m128 r = fload2x4(&neighu[index - start + 1]);
      __m128 d = fload2x4(&neighv[index + width]);
      __m128 l = fload2x4(&neighu[index - start]);
      __m128 result = fmul(ratio, fadd(fsub(r, l), fsub(d, u)));
...
static void project_rb_step2(unsigned int n, grid_color color,
  ...
  for (unsigned int i = 1; i <= n; ++i, start = 1 - start) {
    for (unsigned int j = start; j < width - (1 - start); j += 4) {
      int index = idx(j, i, width);
      __m128 oldu = fload2x4(&sameu[index]);
      __m128 oldv = fload2x4(&samev[index]);
      __m128 u = fload2x4(&neighu0[index - width]);
      __m128 r = fload2x4(&neighu0[index - start + 1]);
      __m128 d = fload2x4(&neighu0[index + width]);
      __m128 l = fload2x4(&neighu0[index - start]);
      __m128 newu = ffnmadd(ratio, fsub(r, l), oldu);
      __m128 newv = ffnmadd(ratio, fsub(d, u), oldv);
